...
| P3-1  | Where parsing runs        | The API only sniffs, stores and enqueues. Parsing, JD fetching and AI structuring run in the worker (`documents` and `analysis` queues). The AI runtime moved to `@cbi/ai-runtime` for both processes                                                                           |
| P3-2  | Upload transport          | Streamed through the API (multipart, memory-buffered, ≤ `UPLOAD_MAX_MB`) to private storage. `STORAGE_PROVIDER=local` in development, Bunny in deployed environments                                                                                                            |
| P3-3  | Blueprint choice          | A bare library role uses the canonical blueprint. A JD or resume produces a private `AI_GENERATED` blueprint (normalised to valid weights), falling back to the canonical one. Admins can promote AI blueprints to roles                                                        |
| P3-4  | Structuring failures      | Not fatal: inputs become `READY` with raw text and `STRUCTURE_UNAVAILABLE`; the interview still uses the text                                                                                                                                                                   |
| P3-5  | Company patterns          | Only admin-verified notes (with source and verifier) reach prompts. Candidates see company names only                                                                                                                                                                           |
| P3-6  | OCR                       | Hook implemented; no OCR provider configured. Scanned PDFs fail with `NO_TEXT` and an `OCR_NEEDED` hint                                                                                                                                                                         |
| P3-7  | Library permissions       | `library.read` / `library.manage` for super, operations and content admins                                                                                                                                                                                                      |
| P4-1  | Engine placement          | `@cbi/interview-engine` is pure (state table, planner, clock, usage). `applySessionEvent` in `@cbi/db` applies an event and its database effects in one transaction; the API and worker both use it                                                                             |
| P4-2  | Rounds storage            | Rounds live in the session's planner state (at most 8) instead of an `interviewRounds` collection, so each turn is one atomic write. Turns (the question ledger) are a separate collection                                                                                      |
| P4-3  | One live interview        | A `live` flag set by the event runner plus a partial unique index on `{userId}`                                                                                                                                                                                                 |
| P4-4  | Turn concurrency          | Each turn runs under a Redis lock per session shared by all API replicas; every session write is also conditional on `stateVersion`                                                                                                                                             |
| P4-5  | Answer idempotency        | `clientMsgId` stored on the turn with a unique index (no separate Redis dedupe window is needed)                                                                                                                                                                                |
| P4-6  | Time-based transitions    | A worker sweep every 15 s: missing heartbeats (45 s) → RECONNECTING, 10 min → PAUSED, 24 h → EXPIRED, stuck COMPLETING → PROCESSING. Time-up while connected is handled by the heartbeat                                                                                        |
| P4-7  | AI outages mid-interview  | Templated fallback questions; failed assessments count as adequate and are flagged for Phase 5 re-evaluation                                                                                                                                                                    |
| P4-8  | Credits                   | Ledger `amount` changes available credits and `reservedDelta` reserved ones; consume and refund share the `settle:<session>` key. The FREE credit is granted lazily on first balance read or start                                                                              |
| P4-9  | Meaningful usage          | ≥ 3 answered questions or ≥ 40 % of the budget with at least one answer. Early ends and platform failures refund                                                                                                                                                                |
| P5-1  | Pipeline shape            | One BullMQ job per stage on an `evaluation` queue, each enqueueing the next (instead of a BullMQ flow); ids `evaluation-<session>-<stage>-<run>`; progress on `session.processing`; a sweep restarts lost or stalled runs                                                       |
| P5-2  | Evidence guard            | AI dimension scores are clamped to ±25 of the evidence-implied score, and dimensions without evidence are not scored. This is the structural defence against injected grading instructions                                                                                      |
| P5-3  | Weights                   | Template category weights split by blueprint competency weights; uncovered categories renormalised; no overall score below 50 % scored weight                                                                                                                                   |
| P5-4  | Always a report           | Every AI stage has a deterministic fallback (live assessments as evidence, evidence-only scores, score-based recommendations)                                                                                                                                                   |
| P5-5  | Scoring policy            | The scoring policy is the one in the session's pinned template version, not a separate `scoringPolicies` collection                                                                                                                                                             |
...
| P7-1  | Speech routing            | STT/TTS go through the AI router like LLM calls (fallback, retries, breaker, slots, metering by audio seconds / characters). Chains: Deepgram Nova-3 → OpenAI transcribe; ElevenLabs Flash v2.5 → OpenAI TTS; mock last in development                                          |
| P7-2  | Per-answer STT            | The whole answer is recorded, uploaded over REST and transcribed, then shown for review before submitting. Live partial captions (`stt:partial`) are deferred: this is replica-safe, gives the candidate a review step, and costs 1–2 s after they stop                         |
| P7-3  | Question audio            | Served over HTTP (`GET …/questions/:id/audio`) from a Redis cache, warmed when the question is asked, instead of `tts:chunk` socket events; replays cost nothing                                                                                                                |
| P7-4  | Server-held transcripts   | A spoken answer is submitted by `voiceTranscriptId`; the server's transcript is the answer text, never the client's                                                                                                                                                             |
| P7-5  | Pre-start checks          | Device check and consent are recorded while the session is READY; `start` walks DEVICE_CHECK → CONSENT_REQUIRED → READY_TO_START in one transaction. Microphone and recorder are required; checks are valid 24 h; consent is versioned                                          |
| P7-6  | No audio storage          | Recordings go to the speech provider and are not stored; synthesized questions live 2 h in Redis. Recording retention and playback come with Phase 8 media                                                                                                                      |
| P7-7  | Degrade to text           | An exhausted speech chain answers 503 SPEECH_UNAVAILABLE and emits `interview:degraded`; switching modes keeps state, clock and ledger, is audited and can be reversed                                                                                                          |
| P7-8  | Scoring spoken answers    | Scoring stays text-only; spoken answers are labelled as automatic transcripts for evidence extraction so transcription slips are not penalised                                                                                                                                  |
| P8-1  | Video = voice + camera    | Video interviews use the voice pipeline (spoken questions, transcribed answers) plus the camera; recording is optional per template policy and consent                                                                                                                          |
| P8-2  | Generic consent           | Versioned consent texts per type and locale (voice processing, recording, integrity) with an append-only `consents` record; replaces the Phase 7 voice-only consent. A new active version asks candidates again                                                                 |
| P8-3  | Segments through the API  | 10 s MediaRecorder segments uploaded through the API to private storage, idempotent by index, accepted until 30 min after the end; failures answer 503 and never touch the interview; finalize marks COMPLETE/PARTIAL/FAILED, and a worker sweep finalizes abandoned recordings |
| P8-4  | Playback by the API       | Signed 5-minute links served by the API, which streams the segments in order (a playable file, no transcoding); this replaces Bunny pull-zone token URLs for segmented recordings. No range requests yet                                                                        |
| P8-5  | Observations, not scores  | Integrity events are stored only with tracking and consent, capped and rate-limited, shown neutrally in reports and admin, and excluded from scoring                                                                                                                            |
| P8-6  | Retention                 | `MEDIA_RETENTION_DAYS_DEFAULT` (90) per recording; the worker deletes expired recordings every 15 min; candidates can delete theirs and admins can purge with a reason (audited first); the record of deletion is kept                                                          |
| P9-1  | Judge contract            | `JudgeAdapter` with HMAC-SHA256 over `ts.METHOD.path.sha256(body)` (±5 min); CodeBegun Judge (target), Judge0 CE batch API (interim, separate host behind a verifying proxy) and a marker-driven mock that never executes code                                                  |
| P9-2  | Coding in the engine      | A CODING round's first question is a bank problem (`question.coding`); submitting code answers the turn, so the planner, clock and ledger are unchanged; follow-ups are ordinary questions                                                                                      |
| P9-3  | Hidden tests              | Stored with the problem, sent only to the judge; candidates see counts and verdicts only                                                                                                                                                                                        |
| P9-4  | Judge down                | Runs answer 503 JUDGE_UNAVAILABLE; submissions are recorded without a run and answer the question; no judge evidence is invented and AI evidence about that code is capped at 0.5 confidence                                                                                    |
| P9-5  | Evidence merge            | Merged in the evidence stage (no new stage): one deterministic item per judged solution (pass rate → strength, confidence 0.9); unsubmitted code is judged by the worker at finalization                                                                                        |
| P10-1 | Campaign pinning          | The role's active blueprint and the template's active version are pinned at creation; campaign interviews use the pinned blueprint (no tailored generation), so every candidate is assessed on the same competencies                                                            |
| P10-2 | Invite tokens             | 144-bit random tokens, only the SHA-256 stored, shown once on create/rotate; rotation invalidates the old link; unknown, malformed and draft tokens all answer 404                                                                                                              |
| P10-3 | Joining                   | One interview per candidate per campaign (unique application index); the limit is claimed with a conditional `$inc` inside the join transaction, so parallel joins never exceed it                                                                                              |
| P10-4 | Sponsored credits         | Claimed atomically at start (`used < total`); a sponsored session never touches the candidate's ledger (credit status SPONSORED); an exhausted budget falls back to the candidate's own credit; a refund returns the unit to the campaign                                       |
| P10-5 | Campaign rules            | A required CAMPAIGN_SHARING consent; the campaign's proctoring replaces the template's; setup limited to the campaign's modes and languages; report visibility decided by the campaign                                                                                          |
| P10-6 | Manual review             | Revisions only: score n+1 and report m+1 with the reviewer's note, overall re-aggregated over the stored weights, confidence carried over; revision 0 is never modified; review views and exports are audited                                                                   |
| P11-1 | Analytics sources         | Clients send only allow-listed interaction events (route patterns, short scalar props, no IP or personal data); business KPIs come from the source collections, never from client events                                                                                        |
| P11-2 | Rollups                   | India-time daily rollups recomputed idempotently (transactional replace per day): the worker refreshes today and yesterday every 15 min, admins backfill ranges; distinct counts over a range are computed live                                                                 |
| P11-3 | Money                     | Dashboard money in INR paise; USD AI cost converted at an admin-set rate; gateway fees estimated from a configured rate; gross margin over net revenue                                                                                                                          |
| P11-4 | KPI targets               | The brief's §56 event list and §57 targets were not received: events follow the funnel in §13 and targets are admin-editable settings with placeholder defaults                                                                                                                 |
| P11-5 | Flags and settings        | Flags with stable per-user rollouts (anonymous visitors only at 100%), settings validated by schema with safe defaults, both cached 15 s per process, changes SUPER-only and audited; maintenance blocks new starts and joins only                                              |
| P11-6 | Candidate Proof           | Built behind `reports.publicProof` (off): hashed expiring revocable links, score-only view of the latest visible revision, 404 everywhere while off                                                                                                                             |
| P12-1 | Load testing              | k6 scenario speaking engine.io frames (HTTP start + Socket.IO room + answers + end) with seeded READY interviews and minted tokens; per-candidate `X-Forwarded-For`; certified on staging (the dev laptop is bound by Docker Desktop round trips)                               |
| P12-2 | Mongo pool and proxy hops | `MONGODB_MAX_POOL_SIZE` (default 100) since every live turn is a short transaction; deployed environments must trust NGINX's hop (`TRUST_PROXY_HOPS ≥ 1`) or per-IP limits collapse onto the proxy                                                                              |
| P12-3 | E2E matrix                | Playwright (desktop Chrome, Pixel 7, WebKit) against compiled services and SPAs rebuilt with the test API URL; axe fails on serious/critical violations; one CI job per browser                                                                                                 |
| P12-4 | Deployment                | Compose on one VPS: NGINX only on 80/443, blue/green API colours swapped by an upstream include after `/readyz`, internal-only Mongo (auth + keyfile) and Redis (password, AOF, noeviction), non-root read-only containers; tag deploys to staging then approved production     |
| P12-5 | Backups                   | Nightly `mongodump` encrypted with `age` to a separate Bunny zone (7 daily + 4 weekly) with a manifest; the monthly drill restores into a network-less scratch container and verifies counts and indexes                                                                        |

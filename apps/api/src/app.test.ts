...
describe('health endpoints', () => {
  ...
  it('GET /readyz is 503 while draining', async () => {
    const res = await request(buildApp({ isDraining: () => true }))
      .get('/readyz')
      .expect(503);
...
describe('request ids', () => {
  ...
  it('reuses a well-formed upstream request id', async () => {
    const res = await request(buildApp()).get('/healthz').set('X-Request-Id', 'nginx-abc12345');
    expect(res.headers['x-request-id']).toBe('nginx-abc12345');
  });

  it('replaces a malformed upstream request id', async () => {
    const res = await request(buildApp()).get('/healthz').set('X-Request-Id', 'bad id<script>');
...
describe('error envelope', () => {
  it('returns NOT_FOUND for unknown v1 routes', async () => {
    const res = await request(buildApp()).get('/api/v1/nope').expect(404);
  ...
  it('returns NOT_FOUND for unknown non-API routes', async () => {
    const res = await request(buildApp()).get('/wp-admin').expect(404);
...
describe('security headers and origin policy', () => {
  it('sets helmet headers and hides the framework', async () => {
    const res = await request(buildApp()).get('/api/v1/x');
  ...
  it('allows server-to-server requests that send no Origin', async () => {
    await request(buildApp()).get('/api/v1/x').expect(404);
  });
});

describe('API docs', () => {
  it('are not served unless enabled', async () => {
    await request(buildApp()).get('/api/docs/openapi.json').expect(404);
  });
  ...
  it('serve the generated OpenAPI document when enabled', async () => {
    ...
    expect(Object.keys(res.body.paths)).toEqual(expect.arrayContaining(['/healthz', '/readyz']));
  });
});

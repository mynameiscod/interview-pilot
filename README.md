...
| **10 — Campaigns & review**     | Done: company campaigns pinning one role blueprint and template version, hashed invite links shown once with rotation, idempotent join with an atomic candidate limit, window/pause/close rules, a company-sharing consent and campaign proctoring, sponsored credits claimed atomically at start, report visibility per campaign, results grid with filters, audited CSV and ZIP package exports, admin interview review with flags and score revisions (new score and report revisions, the AI original untouched), campaign landing and join screens, admin campaign and review screens. See [docs/architecture/campaigns-and-review.md](docs/architecture/campaigns-and-review.md)                                                                                     |
| **11 — Admin analytics & ops**  | Done: allow-listed privacy-safe product analytics, India-time daily rollups from source collections (worker and admin backfill), dashboard KPIs with targets, cohort funnel and provider health, AI cost and margin views, system health with worker heartbeats and stuck interviews, queue view with audited retries, feature flags with stable rollouts, validated system settings (maintenance mode, exchange rate, KPI targets), Candidate Proof links behind a flag (off), admin dashboard/costs/system screens, candidate tracker, maintenance banner and proof pages. See [docs/architecture/analytics-and-operations.md](docs/architecture/analytics-and-operations.md)                                                                                            |
| **12 — Hardening & launch**     | Done (launch pending staging): k6 load test of 200 concurrent interviews (100% completed, 0 errors; latency to be certified on staging), Playwright e2e on desktop Chrome, mobile Chrome and WebKit with axe accessibility checks, security review with fixes (dependency override, trusted-proxy guard, sign-in return path, contrast, CI on master), production Compose with NGINX/TLS/CSP and blue/green deploys, VPS provisioning, encrypted off-site backups with a verified restore drill, deploy workflow with approval, runbooks and launch checklist. See [docs/deployment/production.md](docs/deployment/production.md), [docs/security/security-review.md](docs/security/security-review.md) and [docs/performance/load-test.md](docs/performance/load-test.md) |
